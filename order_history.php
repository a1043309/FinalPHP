<?php
	...
	if (isset($_GET['purchase_code'])) {
		...
		$ratetoseller = $q;
		echo $q;
		$sql3 = "UPDATE purchase SET RateToSeller = $ratetoseller  WHERE Purchase_Code = '$purchase_code'";
		$result3 = mysqli_query($Link,$sql3);
		echo "<script>alert($q);location.href='order_history.php';</script>";
...
<body>
	<div class="user_wrap">
		...
		<div class="myorder_content1">
			...
			<a href="order_history.php" class="htext2-2">已購買的商品</a>
		...
		<div class="myorder_content2">
			...
			<?php
				$sql2 = "SELECT * FROM purchase, product WHERE (product.P_Code = purchase.P_Code AND purchase.Buyer_ID = '$UID' AND purchase.isReceive = 1)";
				$result2 = mysqli_query($Link,$sql2);
				while ($row2 = mysqli_fetch_assoc($result2)) {?>
					<tr>
						<td><?php echo $row2["Seller_ID"]; ?></td>
						...
						<td>已完成</td>
						<?php
							echo "<form action='order_history.php' method='get' id='rate'>";
							$purchase_code=$row2["Purchase_Code"];
						 	echo "<input type='hidden' name='purchase_code' value=$purchase_code>"; 
						 	 ?>

						<td style="padding: 10px 10px;"><?php echo "<input type='number' size='1' style='width:30px;' name='ratetoseller' value='$row2[RateToSeller]'>/10"; ?></td>
						<?php 
